      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-5 md:px-8 py-16 sm:py-24 pt-20 sm:pt-32">
        ...
        >
          ...
          <motion.div
            ...
            className="inline-flex items-center gap-2 pr-3 pl-1 py-1 backdrop-blur-md bg-white/10 border border-white/20 rounded-full mb-4 shadow-2xl"
          >
            <div className="flex items-center justify-center w-5 h-5 rounded-full bg-emerald-500/20">
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse shadow-[0_0_8px_rgba(52,211,153,0.8)]" />
            </div>
            <span className="text-[9px] md:text-[10px] font-bold text-[#F5F1E8] tracking-[0.2em] uppercase">
              {program.location}
            </span>
          </motion.div>

          <h1 className="font-black text-white leading-[1.2] tracking-tight mb-3 md:mb-4">
            {(() => {
          ...
          <div className="flex flex-wrap items-center gap-4">
            {program.status === 'open' && (
              <button
                ...
                className="inline-flex h-9 md:h-12 items-center justify-center gap-2 rounded-full bg-emerald-600 px-5 md:px-8 text-[11px] md:text-sm font-bold text-white transition-all hover:bg-emerald-500 active:scale-95 shadow-lg shadow-emerald-900/40 group"
              >
                Daftar Sekarang
                <ArrowRight
                  size={16}
            ...
            {program.status === 'closed' && (
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-red-500/20 border border-red-500/30 rounded-full">
                <span className="text-xs font-bold text-red-300">Pendaftaran Ditutup</span>
              </div>
            )}

            {program.status === 'ongoing' && (
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-amber-500/20 border border-amber-500/30 rounded-full">
                <span className="text-xs font-bold text-amber-300">Program Sedang Berlangsung</span>
              </div>
            )}
          </div>
        </motion.div>
