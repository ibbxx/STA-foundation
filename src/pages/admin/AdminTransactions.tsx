        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-2">
          <div className="space-y-3 rounded-xl border border-slate-200 p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-sm font-bold text-slate-900">QRIS Manual (Gambar)</p>
                <p className="mt-1 text-xs text-slate-500">Gambar ini tampil ke donatur saat memilih QRIS statis.</p>
              </div>
          ...
          <div className="space-y-3 rounded-xl border border-slate-200 p-4 lg:col-span-2">
            <div className="flex items-center gap-2">
              <QrCode size={16} className="text-slate-500" />
              <div>
                <p className="text-sm font-bold text-slate-900">QRIS Raw String (Dinamis)</p>
                <p className="mt-1 text-xs text-slate-500">
                  Paste string EMV TLV dari QRIS merchant (diawali "000201..."). Jika diisi, sistem akan otomatis men-generate QR dinamis berisi nominal.
                </p>
              </div>
            </div>
            <textarea
              value={paymentSettings.qris_raw_string}
              onChange={(event) => setPaymentSettings((current) => ({ ...current, qris_raw_string: event.target.value }))}
              rows={3}
              placeholder="00020101021126570016..."
              className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 font-mono text-xs text-slate-700 focus:border-zinc-900 focus:outline-none focus:ring-1 focus:ring-zinc-900"
            />
            {paymentSettings.qris_raw_string.trim() ? (() => {
              const validation = validateQrisRawString(paymentSettings.qris_raw_string.trim());
              return (
                <p className={`text-xs font-semibold ${validation.valid ? 'text-emerald-600' : 'text-rose-600'}`}>
                  {validation.valid ? '✓ Format valid' : `✗ ${validation.error}`}
                </p>
              );
            })() : null}
          </div>
                    ...
                    <td className="px-6 py-4">
                      ...
                      {transaction.unique_code > 0 ? (
                        <p className="text-xs text-emerald-600 font-semibold mt-0.5">
                          +{transaction.unique_code} → {formatCurrency(transaction.final_amount ?? transaction.amount)}
                        </p>
                      ) : null}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {[
              ...
              ...(selectedTransaction.unique_code > 0 ? [
                ...
                ['Nominal Final', formatCurrency(selectedTransaction.final_amount ?? selectedTransaction.amount)],
              ] : []),
